...
import { Search, Users, Building2, DollarSign, Star, ArrowRight, Package } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <Badge variant="secondary" className="gap-1 bg-growi-blue/10 text-growi-blue">
          {campaignManagers.reduce((acc, m) => acc + m.campaigns.filter(c => c.status === "active").length, 0)} active campaigns
        </Badge>
      </motion.div>
          ...
          <motion.div key={manager.id} variants={staggerItem}>
            <Card className="h-full transition-all hover:border-growi-blue/50 hover:shadow-md">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {manager.campaigns.filter(c => c.status === "active").map((campaign) => (
                      <Link 
                        key={campaign.id} 
                        href={`/influencer/campaigns?apply=${campaign.id}`}
                      >
                        <motion.div
                ...
                <Link href={`/influencer/campaigns?manager=${manager.id}`}>
                  <Button
                    variant="outline"
